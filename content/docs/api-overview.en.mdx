---
title: API 概览
description: RouteLab 当前公开网关的兼容接口、扩展字段与常见错误约定。
---

RouteLab 的公开数据面以 **OpenAI 兼容 API** 为核心，目标是让现有 SDK 和调用方式可以尽量直接复用。
...
在兼容 OpenAI 的前提下，RouteLab 增加了一些可选字段用于路由控制。
...
RouteLab 的目标之一是让调用不再是黑盒，因此非流式响应通常会保留这些信息：
...
1. 把 `baseURL` 改成 RouteLab 网关
2. 换成 RouteLab 的 API Key
3. 把模型名换成 RouteLab 暴露的逻辑模型名
4. 需要时再逐步增加 `routing_preference` 或 `provider`

如果你想继续理解“平台为什么选中这条 route”，下一篇读 [路由与模型](./routing-and-models)。
