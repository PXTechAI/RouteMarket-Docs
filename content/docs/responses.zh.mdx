---
title: Responses
description: The newer RouteLab response-style API surface and when to use it.
---

RouteLab also exposes a response-style API surface in addition to chat completions.
...
curl https://api.routemarket.ai/api/gateway/v1/responses \
  ...
  -d '{
    "model": "gpt-5.4",
    "input": "Explain RouteLab in one short paragraph."
  }'
```

## What stays the same

Even though the request shape differs, the main RouteLab runtime ideas stay the same:

- you still request a logical model
- your key policy still affects what can be used
- RouteLab may still route across multiple candidate sources
- pricing, usage, and route metadata still matter

## RouteLab-specific controls
...
If you are starting from existing OpenAI-compatible code, read [Chat Completions](./chat-completions).
